-- COMPLETE LIFE MANAGER V2 DATABASE MIGRATION
-- This script creates ALL required tables in the correct order
-- Run this SQL in your Supabase SQL editor

-- First, let's check what tables already exist to avoid conflicts
DO $$
BEGIN
    RAISE NOTICE 'Starting Life Manager v2 Complete Database Migration...';
END $$;

-- =====================================================
-- PART 1: MISSING TABLES FROM FIRST MIGRATION
-- =====================================================
...
-- Memory Images table (for memories functionality)
...
DROP POLICY IF EXISTS "Users can view memory images for their memories" ON memory_images;
DROP POLICY IF EXISTS "Users can insert memory images for their memories" ON memory_images;
DROP POLICY IF EXISTS "Users can update memory images for their memories" ON memory_images;
DROP POLICY IF EXISTS "Users can delete memory images for their memories" ON memory_images;

CREATE POLICY "Users can view memory images for their memories" ON memory_images FOR SELECT USING (
    EXISTS (SELECT 1 FROM memories WHERE memories.id = memory_images.memory_id AND memories.user_id = auth.uid())
);
CREATE POLICY "Users can insert memory images for their memories" ON memory_images FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM memories WHERE memories.id = memory_images.memory_id AND memories.user_id = auth.uid())
);
CREATE POLICY "Users can update memory images for their memories" ON memory_images FOR UPDATE USING (
    EXISTS (SELECT 1 FROM memories WHERE memories.id = memory_images.memory_id AND memories.user_id = auth.uid())
);
CREATE POLICY "Users can delete memory images for their memories" ON memory_images FOR DELETE USING (
    EXISTS (SELECT 1 FROM memories WHERE memories.id = memory_images.memory_id AND memories.user_id = auth.uid())
);
...
-- Memory Images indexes
CREATE INDEX IF NOT EXISTS memory_images_memory_id_idx ON memory_images(memory_id);
CREATE INDEX IF NOT EXISTS memory_images_order_idx ON memory_images(order_index);
...
DO $$
BEGIN
    RAISE NOTICE '✅ Life Manager v2 Complete Database Migration SUCCESSFUL!';
    RAISE NOTICE '🎉 All 14 tables created with proper RLS policies and indexes!';
    RAISE NOTICE '🚀 Your Life Manager Dashboard is now 100%% ready for deployment!';
END $$;
